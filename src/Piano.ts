...
export class Piano {
	...
	public getKey(midiNote: number): Mesh | undefined {
	...
	private createKeys(): void {
		const whiteKeyMaterial = new MeshStandardMaterial({ color: 0xffffff, roughness: 0.5, metalness: 0.1 });
		const blackKeyMaterial = new MeshStandardMaterial({ color: 0x222222, roughness: 0.5, metalness: 0.1 });

		const whiteKeyGeom = new BoxGeometry(WHITE_KEY_WIDTH, WHITE_KEY_HEIGHT, WHITE_KEY_DEPTH);
		const blackKeyGeom = new BoxGeometry(BLACK_KEY_WIDTH, BLACK_KEY_HEIGHT, BLACK_KEY_DEPTH);
		...
		for (let i = 0; i < NUM_KEYS; i++) {
			...
			let keyMesh: Mesh;
			if (isBlackKey) {
				keyMesh = new Mesh(blackKeyGeom, blackKeyMaterial.clone());
				...
				keyMesh = new Mesh(whiteKeyGeom, whiteKeyMaterial.clone());
